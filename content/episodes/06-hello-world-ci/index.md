...
objectives = ['Add CI/CD to your project.']
questions = ['How do I run a simple GitLab CI job?']
teaching = 5
...
{{< callout type="note" title="Feature Branches" >}}
...
![CI/CD Configuration Button](fig/ci-cd-configuration-button.png)

which will link to the newly added `.gitlab-ci.yml`. But wait a minute, there's also a big red `x` on the page too!

![Commit's CI/CD Failure Example](fig/ci-cd-commit-failure.png)
...
![CI/CD Failure YAML Invalid](fig/ci-cd-commit-failure-yaml-invalid.png)
...
![CI/CD Failure YAML Invalid Pipeline page](fig/ci-cd-commit-failure-yaml-invalid-pipeline-page.png)

{{< callout type="note" title="Validating CI/CD YAML Configuration" >}}
...
{{< callout type="note" title="But what's a linter?" >}}
...
![Work In Progress](fig/work-in-progress.png)
{{< /callout >}}
...
![CI/CD Hello World Lint](fig/ci-cd-hello-world-lint.png)
...
![CI/CD Hello World Success Output](fig/ci-cd-hello-world-success-output.png)
